/* 
   board.cpp              (written by Yunpeng Li)
   ...
   method, but instead access the another board array directly.
*/
...
Board* makeBoard(char *guiArg) {
  assert(strlen(guiArg) == GUI_ARG_LENGTH);
  Board *b = (Board*)malloc(sizeof(Board));
  int discCount = 0;
  for(int i=0; i<64; i++)
    if(guiArg[i] - '0' != EMPTY)  // guiArg is a "text" char string
      discCount++;
  if(discCount < 4) { // invalid board
    printf("Invalid board: fewer than 4 discs\n");
    exit(101);
  }
  char *a = b->a[discCount-4];
  for(int i=0; i<64; i++)
    a[i] = guiArg[i] - '0';
  b->mirrored = false;
  b->n = discCount - 4;
  ...
  b->top = b->n;
  b->wt = guiArg[64] - '0';
  if(b->wt != BLACK && b->wt != WHITE) {
    printf("Invalid whose_turn info\n");
    exit(102);
  }
  b->moves[(int)b->n] = 10 * (guiArg[65] - '1') + (guiArg[66] - '0');
              // use 10+actual_move to gurrantee two digits
  if(b->moves[(int)b->n] < 0 || b->moves[(int)b->n] > 63) {
    if(b->moves[(int)b->n] != GUI_PASS - 10) { // use code "99" for pass
      printf("Invalid move\n");
      exit(103);
    }
    b->moves[(int)b->n] = PASS;
  }
  return b;
}
...
void setPiece(Board *board, char place, char color) {
  board->a[(int)board->n][(int)place] = color;
}

/* get color */
char getPiece(Board *board, char place) {
  return board->a[(int)board->n][(int)place];
}

void flipPiece(Board *board, char place) {
  char &p = board->a[(int)board->n][(int)place];
  p = OTHER(p);
}
/*-----------------------------*/
...
void makeMove(Board *b, char x, char y) {
  ...
  if(b->m > b->top || b->moves[(int)b->m] != place) {
    b->top = b->m;  // whenever a different move is made, no 'redo' possible
    b->moves[(int)b->m] = place;  // record this move
  }
}
...
void makePass(Board *b) {
  b->m += 1;
  b->wt = OTHER(b->wt);
  if(b->m > b->top || b->moves[(int)b->m] != PASS) {
    b->top = b->m;
    b->moves[(int)b->m] = PASS;
  }
}
...
void countPieces(Board *b, char *nb, char *nw) {
  countPieces(b->a[(int)b->n], nb, nw, b->n + 4); // has 4 pieces when game starts.
}

/* print out the board */
void printBoard(Board *b, bool *legalMoves) {
  char *a = b->a[(int)b->n];
  char lastMove = b->moves[(int)b->m];
  char place;
  const char *s;
  char nb, nw;
  countPieces(b, &nb, &nw);
